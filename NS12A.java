...
public class NS12A{
 ...
 void modularity(){  // (9.12)
	...
	System.out.println("Number of communities "+numberOfCommunities + " M1-M2 " + (M1 - M2));
	System.out.println();
 }
 ...
 void louvainStep1(){
	System.out.println("Modularity 1"); 
	modularity();  // initial modularity is negative
	while(louvainRound()){ 
           System.out.println("Modularity 2");
 ...
 void communities(){
	...
	// for large networks, you may only want to present those with size > 10
   	System.out.println("Communities with neighbhourhood greater than 0");
	for (int i = 0; i < N; i++) {
		if(sizes[i]!=0){
			System.out.println(labels.get(i)+" "+sizes[i]);
		}
	}
 }
